...
uint8_t *i2c_buf_rx;
uint8_t i2c_buf_rx_len;
volatile uint32_t i2c_buf_rx_count;

uint8_t *i2c_buf_tx;
uint8_t i2c_buf_tx_len;
...
void i2c_slave_init(uint8_t i2c_addr)
{
    ...
    i2c_set_rx_buffer(0, 0);

    i2c_set_tx_buffer(0, 0);
...
void I2C1_IRQHandler(void)
{
    ...
    else if ((I2C_InterruptStatus & I2C_ISR_RXNE) == I2C_ISR_RXNE)
    {
        ...
        {
            if (i2c_reg<i2c_buf_rx_len)
                i2c_buf_rx[i2c_reg++] = (I2C1->RXDR)&0xFF;
            else
                dummy = I2C1->RXDR;
        }
        i2c_op = I2C_OP_RX;
    ...
    else if ((I2C_InterruptStatus & I2C_ISR_TXIS) == I2C_ISR_TXIS)
    {
        ...
        if (i2c_reg<i2c_buf_tx_len)
            I2C1->TXDR = i2c_buf_tx[i2c_reg++];
            //I2C1->TXDR = i2c_reg;
        else
            I2C1->TXDR = 0xee;
        i2c_op = I2C_OP_TX;
